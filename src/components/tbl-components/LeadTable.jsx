...
const LeadTable = () => {
  ...
  const fetchLeads = async () => {
    try {
      const response = await axios.get('http://localhost:5028/api/Lead');
  ...
  const handleDelete = async (LeadId) => {
    try {
      await axios.delete(`http://localhost:5028/api/Lead/${LeadId}`);
      setData(data.filter(record => record.LeadId !== LeadId));
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditMode) {
        ...
        await axios.put(`http://localhost:5028/api/Lead/${currentRecord.LeadId}`, currentRecord);
        setData(data.map(item => (item.LeadId === currentRecord.LeadId ? currentRecord : item)));
      } else {
        // Add new record
        const response = await axios.post('http://localhost:5028/api/Lead', currentRecord);
          ...
          {data.map((record) => (
            <tr key={record.id} className="border-t">
              ...
              <td className="px-4 py-2">
                ...
                <button className="bg-red-500 text-white px-2 py-1 rounded" onClick={() => handleDelete(record.LeadId)}>Delete</button>
              </td>
            </tr>
          ))}
        </tbody>
